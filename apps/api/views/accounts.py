...
class UserLoginAPIView(APIView):
    ...
    @extend_schema(
        request=UserLoginSerializer, responses={200: UserLoginResponseSerializer}
    )
    def post(self, request):
...
class VerifyOTPAPIView(APIView):
    ...
    @extend_schema(request=VerifyOTPSerializer, responses={200: {"type": "object"}})
    def post(self, request):
        serializer = VerifyOTPSerializer(data=request.data)
        if serializer.is_valid():
            user = get_object_or_404(
                CustomUser, id=serializer.validated_data["user_id"]
            )
