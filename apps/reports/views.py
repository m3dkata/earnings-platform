    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.is_staff:
            ...
            reports = Report.objects.filter(
                ...
                'reportoperation_set__operation'
            )
            ...
            month_year = self.request.GET.get('month_year', timezone.now().strftime('%Y-%m'))
...
class ReportCreateView(LoginRequiredMixin, WorkHourRestrictionMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.POST:
            ...
            context['formset'] = ReportOperationFormSet(queryset=ReportOperation.objects.none())
        return context
    ...
    def form_valid(self, form):
        ...
        if not formset.is_valid():
            messages.error(self.request, 'Please check the operations data and try again.')
        ...
        report.number = report.generate_number()
...
class ReportUpdateView(LoginRequiredMixin, WorkHourRestrictionMixin, UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['operations'] = ReportOperation.objects.filter(report=self.object).select_related('operation')
        ...
        if not self.request.user.is_staff and self.object.status != 'APPROVED':
            if self.request.POST:
                context['formset'] = ReportOperationFormSet(self.request.POST, queryset=context['operations'])
            else:
                context['formset'] = ReportOperationFormSet(queryset=context['operations'])
        return context
    ...
    def post(self, request, *args, **kwargs):
        ...
        if request.user.is_staff:
            if 'approve' in request.POST:
                ...
                messages.success(self.request, f'Report {self.object.status.lower()} successfully!')
