        ...
        if date and employee:
            if Report.objects.filter(employee=employee, date=date).exists():
                raise ValidationError(
                    "Report for this employee and date already exists"
                )
        return cleaned_data
            ...
            if self.instance and self.instance.pk:
                if (
                    ReportOperation.objects.filter(
                        report=self.instance.report, operation=operation
                    )
                    .exclude(pk=self.instance.pk)
                    .exists()
                ):
                    raise ValidationError(
                        "This operation is already added to the report"
                    )
        return cleaned_data
...
ReportOperationFormSet = modelformset_factory(
    ...
    can_delete=True,
    validate_max=True,
    error_messages={"too_many_forms": "Maximum 10 operations allowed per report"},
