...
class Report(models.Model):
    ...
    def generate_number(self):
        last_report = Report.objects.all().order_by('-number').first()
        if last_report:
            return last_report.number + 1
        return 1

    number = models.IntegerField(unique=True, verbose_name='Report Number', editable=False)
