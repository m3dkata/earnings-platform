...
from .models import CustomUser, EmailVerification, EmployeeUser, FaceDescriptor, PendingUser, ArchivedUser
...
class CustomUserAdmin(UserAdmin):
    list_display = ('username', 'email', 'first_name', 'last_name', 'is_active', 'is_employee', 'is_staff', 'is_superuser')
    list_filter = ('is_active', 'is_employee', 'is_staff', 'is_superuser')

    fieldsets = UserAdmin.fieldsets + (
        ('Security', {'fields': ('is_2fa_enabled', 'is_email_otp_enabled',)}),
        ('Employee Status', {'fields': ('is_employee', 'is_archived',)}),
    )
...
class EmployeeAccount(UserAdmin):
    ...
    def get_queryset(self, request):
        return super().get_queryset(request).filter(is_active=True, is_employee=True, is_archived=False)
...
class PendingAccount(UserAdmin):
    ...
    fieldsets = (
        ('Employee Status', {'fields': ( 'is_active', 'is_employee', 'is_archived',)}),
    ) 
    
    def get_queryset(self, request):
        return super().get_queryset(request).filter(is_active=False, is_employee=True, is_archived=False)
...
class ArchivedAccount(UserAdmin):
    ...
    def get_fieldsets(self, request, obj=None):
        employee_status = (
            ('Employee Status', {'fields': ('is_archived', 'is_employee',)}),
        )
        return employee_status + UserAdmin.fieldsets
