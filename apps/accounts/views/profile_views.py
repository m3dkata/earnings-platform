...
class ProfileView(LoginRequiredMixin, PaginationMixin, BaseTemplateView):
    ...
    def get_activity_logs(self):
        if self.request.user.is_staff:
            ...
            logs = (
                LogEntry.objects.filter(content_type__model__in=content_types)
    ...
    try:
        if request.user.is_2fa_enabled:
            ...
            device = TOTPDevice.objects.create(
                user=request.user, name=f"2FA for {request.user.username}"
            )
...
def toggle_email_otp(request):
    try:
        ...
        messages.success(
            request,
            "Email OTP has been "
            + ("enabled" if request.user.is_email_otp_enabled else "disabled"),
        )
    except Exception:
