...
from ..utils import send_otp_email, send_welcome_email, send_password_reset_email, generate_otp, rate_limit
...
class SignUpView(CreateView):
    ...
    def form_valid(self, form):
        try:
            ...
            messages.info(self.request, "Your account has been created and is pending admin approval")
        ...
        try:
            ...
            if user.is_2fa_enabled or user.is_email_otp_enabled:
                if user.is_email_otp_enabled:
                    ...
                    messages.info(self.request, "Please check your email for verification code")

                if user.is_2fa_enabled:
                    messages.info(self.request, "Or enter your 2FA code from authenticator app.")

                return redirect('verify_login_email' if user.is_email_otp_enabled else 'verify_login_2fa')
...
def verify_login_email(request):
    ...
    return render(request, 'accounts/verify_login_email.html', {
        'user_has_2fa': CustomUser.objects.get(id=user_id).is_2fa_enabled,
        'user_has_email_otp': CustomUser.objects.get(id=user_id).is_email_otp_enabled
    })