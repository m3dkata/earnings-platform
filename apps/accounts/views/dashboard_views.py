...
class DashboardView(LoginRequiredMixin, TemplateView):
    ...
    def get_base_query(self, date):
        return (
            ...
            .prefetch_related("reportoperation_set")
            .filter(date__year=date.year, date__month=date.month, status="APPROVED")
        )
    ...
    def get_monthly_stats(self, date, user):
        ...
        if user.is_staff:
            return {
                ...
                "earnings": stats["total_earnings"] / employee_count
                if stats["total_earnings"]
                else 0,
    ...
    def get_chart_data(self):
        ...
        return {
            ...
            "sums": [
                float(
                    stat["daily_sum"] / stat["employee_count"]
                    if self.request.user.is_staff
                    else stat["daily_sum"] or 0
                )
        ...
        context.update(
            {
                ...
                "operations_change": current_stats["operations"]
                - last_month_stats["operations"]
                if last_month_stats["operations"]
                else 0,
                "total_earnings": current_stats["earnings"],
                "earnings_change": current_stats["earnings"]
                - last_month_stats["earnings"]
                if last_month_stats["earnings"]
                else 0,
                "work_days": current_stats["work_days"],
                "work_days_change": current_stats["work_days"]
                - last_month_stats["work_days"]
                if last_month_stats["work_days"]
                else 0,
                "avg_percent": current_stats["avg_percent"],
                "percent_change": (
                    (current_stats["avg_percent"] - last_month_stats["avg_percent"])
                    / last_month_stats["avg_percent"]
                    * 100
                )
                if last_month_stats["avg_percent"]
                else 0,
