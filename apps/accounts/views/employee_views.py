...
class BaseEmployeeListView(ListView):
    ...
    def get_queryset(self):
        return (
            ...
            .defer("user__password", "user__last_login", "user__date_joined")
        )
...
class ActiveEmployeeListView(BaseEmployeeListView):
    ...
    def get_queryset(self):
        return (
            Employee.objects.filter(user__is_active=True, user__is_archived=False)
            .select_related("user", "workshop")
            .order_by("number")
        )
...
class InactiveEmployeeListView(BaseEmployeeListView):
    ...
    def get_queryset(self):
        return CustomUser.objects.filter(
            is_active=False, is_archived=False, is_employee=True
...
class ArchivedEmployeeListView(BaseEmployeeListView):
    ...
    def get_queryset(self):
        return (
            Employee.objects.filter(user__is_archived=True)
            .select_related("user", "workshop")
            .order_by("number")
        )
...
def toggle_employee_status(request, employee_id):
    ...
    if request.method == "GET":
        ...
        return render(
            request, "employees/activate_modal.html", {"form": form, "user": user}
        )
    ...
    if action == "activate":
        ...
        if form.is_valid():
            ...
            try:
                ...
                messages.warning(
                    request, "Employee group not found. Please create the group."
                )
            user.is_active = True
            user.is_archived = False
            user.save()
            messages.success(
                request, f"Employee {user.get_full_name()} activated successfully"
            )
        ...
        user.delete()
        messages.success(
            request, f"Employee {user.get_full_name()} deleted successfully"
        )
        return redirect("archived_employees")

    user.save()
    messages.success(
        request, f"Employee {user.get_full_name()} status updated successfully"
    )
    return redirect(request.META.get("HTTP_REFERER", "employee_list"))
