...
class OperationListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['page_range'] = range(
            max(page.number - 2, 1),
            min(page.number + 2, paginator.num_pages) + 1
        )
        return context
...
class OperationCreateView(CreateView):
    ...
    def form_invalid(self, form):
        messages.error(self.request, 'Operation creation failed. Please check the form.')
...
class OperationDeleteView(DeleteView):
    ...
    def form_valid(self, form):
        self.object.delete()
        messages.success(self.request, 'Operation deleted successfully')
        return redirect(self.success_url)
...
class RateDeleteView(DeleteView):
    ...
    def form_valid(self, form):
        try:
            self.object.delete()
            messages.success(self.request, 'Rate deleted successfully')
            return redirect(self.success_url)
        except ProtectedError:
            messages.error(self.request, 'Cannot delete this rate because it is being used by operations')
            return redirect(self.success_url)
