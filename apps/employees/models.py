...
class Employee(models.Model):
    ...
    position = models.CharField(
        max_length=100, choices=POSITION_CHOICES, verbose_name="Position"
    )
    workshop = models.ForeignKey(
        Workshop, on_delete=models.PROTECT, verbose_name="Workshop"
    )
...
class Leave(models.Model):
    ...
    def __str__(self):
        return (
            f"{self.employee.user.get_full_name()} - {self.leave_type} / {self.status}"
        )
