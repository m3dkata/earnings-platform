...
class PayrollTests(TestCase):
    ...
    def test_payroll_pdf_view(self):
        self.client.login(username='staffuser', password='StrongPass123!')
        response = self.client.get(reverse('payroll-pdf', kwargs={'pk': self.payroll.pk}))
        self.assertEqual(response.status_code, 200)
    ...
    def test_payroll_create_view(self):
        ...
        self.assertTrue(Payroll.objects.filter(employee=self.employee, bank_transfer=1500).exists())
